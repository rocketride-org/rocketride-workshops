# Cody Rider — Workshop Exercise

This is the follow-along document for the **coding-agent** workshop. You will build a multi-agent coding pipeline that takes a typed / audio / image request from the UI and produces a working software project on disk.

The UI, API, and runtime are already wired. You build the pipeline only. The empty starter lives at `api/app/pipelines/coding-agent.pipe` — open it in VS Code with the RocketRide extension to get the visual canvas.
...
- Typed text from the chat UI → `webhook_1.text` lane.
- Audio voice notes → `webhook_1.audio` → `audio_transcribe_1` → text.
- Uploaded images → `webhook_1.image` → `ocr_1` → text.

All three text streams funnel into a single `question` node that normalizes the result into the `questions` lane the rest of the pipeline reads.

This section is about the **lane model**. The source node emits lanes; parser nodes consume one lane and emit another. Downstream agents only care about `questions`.
...
- `Web Hook.audio` → `Transcribe.audio`
- `Web Hook.image` → `OCR.image`
- `Transcribe.text` → `Question.text`
...
None — source and parser nodes are pure config.

### Verify

Save the pipe. Confirm the runtime reloads with no validation errors in the Connection Manager.
...
Paste verbatim into `Project Manager → Config → Default → System Prompt`:
...
Send "hi" through the chat — you should get a conversational reply (the trivial path). Send "build me a hello world page" and the PM will try to delegate, fail (no sub-agents yet), and likely apologize. That's expected. Move on.
...
Wire Engineer 1 fully, then duplicate the node + tool stack for Engineer 2 and Engineer 3. The prompts only differ in the engineer number and branch prefix.

> **Tip**: copy-paste the Engineer 1 node block in the raw `.pipe` JSON file three times and rename IDs. Faster than dragging from the canvas.

### Nodes to add (per engineer, repeat ×3)
...
- `Shell Eng N`, `Git Eng N` **control** lanes `tool` from `Engineer N`.
...
Replace `{N}` with `1`, `2`, or `3` when pasting into each Engineer node:
...
Re-send "build me a todo list app". Architect's OWNERSHIP.md will pick 1, 2, or 3 engineers; you should see the matching number of branches get created in parallel. The PM will then try to review and merge, fail (no Reviewer or DevOps yet), and apologize.
...
- **DevOps** owns `main`. Bootstrap (one-time per repo), scaffolds per-project infra, commits the Architect's design docs, runs a pre-merge smoke test, merges feature branches, commits the tests QA emits. The only role with write access to `main`.
- **QA** has no file-write tool. Emits test files as `(path, content)` pairs in its reply (Phase A); DevOps commits them; QA then runs the test command (Phase B). Two-phase split.
- **Reviewer** is read-only. Sits BETWEEN engineer fan-out and DevOps merge — inspects each `feat/eng*` branch against ARCHITECTURE.md + OWNERSHIP.md and returns either `APPROVED` (DevOps may merge) or `REQUEST_FIXES` (PM re-tasks the engineer).

Once all three are wired, full order of operations is:
...
Same control-lane pattern as the Engineers. All three sub-agents attach `deepagent` control to `Project Manager`. Each tool attaches `tool` control to its parent sub-agent. Each Anthropic node attaches `llm` control to its parent sub-agent.
...
# readOnlyMode left absent — defaults to true. Reviewer cannot write.
```
...
Paste verbatim into `DevOps → Config → Default → System Prompt`:
...
Paste verbatim into `QA → Config → Default → System Prompt`:
...
Paste verbatim into `Reviewer → Config → Default → System Prompt`:
...
1. Send "hi" and get a conversational reply.
2. Send "build me a hello world page" and get a working file tree under `api/.output/hello-world/` with at least one commit on `main`.
3. Send "build me a notes app with a list page, a new-note form, and SQLite storage with tests" and see the full workflow execute (architect → fan-out → reviewer → merge → QA), with `api/.output/notes-app/` containing a working project and a tests directory.

...you've built the full pipeline. Compare your `coding-agent.pipe` against `workshops/coding-agent/solution/api/app/pipelines/coding-agent.pipe` if you want to see one possible canonical wiring.

## Going further

The single-agent baseline lives at `api/app/pipelines/coding-agent-single.pipe`. It does the same job with one Deep Agent and a shared toolset. Compare the two to see the trade-off: the multi-agent fan-out is faster on parallelizable work, more expensive on tokens, and produces a cleaner git history. The single agent is cheaper and simpler, slower on big changes.
